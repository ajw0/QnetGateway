 ...
 *
 *   CQnetBT::GetBTData() is based on some code that is...
...
int CQnetBT::OpenBT()
{
	int fd = open(BT_DEVICE.c_str(), O_RDWR | O_NOCTTY | O_NDELAY, 0);
	if (fd < 0)
	{
		printf("Failed to open device [%s], error=%d, message=%s\n", BT_DEVICE.c_str(), errno, strerror(errno));
		return -1;
	}
...
REPLY_TYPE CQnetBT::GetBTData(unsigned char *buf)
{
...
void CQnetBT::Run()
{
	...
	while (keep_running)
	{
		...
		{
			auto res = GetBTData(buf);
			if (res != RT_TIMEOUT)
			{
				timeout_count = 0;  // Reset on any real data
				if (LOG_DEBUG) DumpPacket("RX", buf);
			}
			switch (res)
			{
				case RT_ERROR:
				...
				case RT_TIMEOUT:	// nothing or 0xff
					// Throttle when getting rapid consecutive timeouts (disconnected device)
					if (++timeout_count > 10)
					{
						// select() is returning immediately instead of blocking
						// Add delay to prevent CPU spin on dead device
...
bool CQnetBT::SendToIcom(const unsigned char *buf)
{
	ssize_t n;
	unsigned int ptr = 0;
...
bool CQnetBT::ProcessGateway(const int len, const unsigned char *raw)
{
...
bool CQnetBT::ProcessBT(const unsigned char *buf)
{
	...
	if (41 == len)  	// header
	{
		...
		{
			printf("ERROR: ProcessBT: Could not write gateway header packet\n");
			return true;
		}
		if (LOG_QSO)
...
bool CQnetBT::ReadConfig(const std::string &cfgFile)
{
	CQnetConfigure cfg;
	...
	cfg.GetValue(bt_path+"_device", type, BT_DEVICE, 7, FILENAME_MAX);
	cfg.GetValue(bt_path+"_ap_mode", type, AP_MODE);

	bt_path.append("_callsign");
	if (cfg.KeyExists(bt_path))
	{
		if (cfg.GetValue(bt_path, type, RPTR, 3, 6))
			return true;
	}
	else
	{
		bt_path.assign("ircddb_login");
		if (cfg.KeyExists(bt_path))
		{
			if (cfg.GetValue(bt_path, estr, RPTR, 3, 6))
				return true;
		}
	}
	int l = RPTR.length();
...
void CQnetBT::calcPFCS(const unsigned char *packet, unsigned char *pfcs)
{
...
int main(int argc, const char **argv)
{
	...
	if ('-' == argv[1][0])
	{
		...
		printf("QnetBT comes with ABSOLUTELY NO WARRANTY; see the LICENSE for details.\n");
		printf("This is free software, and you are welcome to distribute it\nunder certain conditions that are discussed in the LICENSE file.\n");
		return 0;
	}

	const char *qn = strstr(argv[0], "qnbt");
	if (NULL == qn)
	{
		fprintf(stderr, "Error finding 'qnbt' in %s!\n", argv[0]);
		return 1;
	}
	qn += 4;
	...
	pbt = std::unique_ptr<CQnetBT>(new CQnetBT(assigned_module));

	if (!pbt)
	{
		fprintf(stderr, "Could not make a CQnetBT\n");
		return EXIT_FAILURE;
	}

	if (pbt->Initialize(argv[1]))
	{
		pbt.reset();
		return EXIT_FAILURE;
	}
